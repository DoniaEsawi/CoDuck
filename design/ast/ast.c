...
AST_Node *new_ast_decl_node(int data_type, ListNode **names){
	// allocate memory
...
AST_Node *new_ast_incr_node(ListNode *entry, int incr_type){
	// allocate memory
...
void ast_print_node(AST_Node *node){
	...
	switch(node->type){
		...
		case DECL_NODE:
			...
			printf("Declaration Node of data-type %d \n",
				temp_decl->data_type);
			break;
		case CONST_NODE:
		...
		case INCR_NODE:
			...
			printf("Increment Node of entry %s being %d \n", 
                 temp_incr->entry->name, temp_incr->incr_type);
...
void ast_traversal(AST_Node *node){
	...
	/* for case */
        ...
        // v->increment = increment;
        // v->for_branch = for_branch;
        // why not traverse the for loop in the order of initialize, condition, increment, for_branch?
