...
{
	// allocate memory
	AST_Node_VAR *v = malloc(sizeof(AST_Node_VAR));
...
AST_Node *new_ast_do_node(AST_Node *condition, AST_Node *do_branch)
{
	...
	// set entries
	v->type = DO_UNTIL_NODE;
	v->condition = condition;
	v->while_branch = do_branch;
...
AST_Node *new_ast_func_call_node(ListNode *entry, AST_Node **params, int num_of_pars)
{
	// allocate memory
...
AST_Node *new_ast_func_decl_node(int ret_type, ListNode *entry)
{
	// allocate memory
...
AST_Node *new_statements_node(AST_Node **statements, int statement_count, AST_Node *statement)
{
	...
	{
		// reallocate memory to be able to add new statement
...
void set_loop_counter(AST_Node *node)
{
	...
	if (strcmp(incr_node->entry->name, assign_node->entry->name))
	{
...
AST_Node *new_ast_decl_params_node(Prameter *parameters, int num_of_pars, Prameter param)
{
	...
	if (parameters == NULL)
	{
		parameters = (Prameter *)malloc(sizeof(Prameter));
		parameters[0] = param;
		num_of_pars = 1;
	}
	// add new declaration
	...
	{
		parameters = (Prameter *)realloc(parameters, (num_of_pars + 1) * sizeof(Prameter));
...
void ast_print_node(AST_Node *node)
{
	...
	AST_Node_VAR *temp_var;
	AST_Node_Statements *temp_statements;
	AST_Node_DO *temp_do_until;
	...
	case IF_NODE:
		...
		printf("If Node with %d elifs and ", temp_if->elseif_count);
	...
	case FOR_NODE:
		...
		printf("For Node with loop counter %s\n", temp_for->counter->name);
		break;
	case WHILE_NODE:
		printf("While Node\n");
		break;
	case DO_UNTIL_NODE:
		printf("Do Until Node\n");
		break;
	case ASSIGN_NODE:
	...
	case INCR_NODE:
		...
		printf("Increment Node of entry %s being %d %d \n",
					 temp_incr->entry->name, temp_incr->incr_type, temp_incr->pf_type);
		// temp_incr->entry->name, temp_incr->incr_type, temp_incr->pf_type);
		break;
	case FUNC_CALL:
		temp_func_call = (struct AST_Node_Func_Call *)node;
		printf("Function Call Node of %s with %d parameters\n", temp_func_call->entry->name, temp_func_call->num_of_pars);
		break;
	case CALL_PARAMS:
	...
	case ARITHM_NODE:
		...
		// declare an array with length 7
		char operators[7][5] = {
				"ADD",
				"SUB",
				"MUL",
		...
		printf("Arithmetic Node of operator %s with result type %s\n", operators[temp_arithm->op], data_types[temp_arithm->data_type]);
		break;
	case BOOL_NODE:
	...
	case EQU_NODE:
		...
		break;
	case VAR_NODE:
		temp_var = (struct AST_Node_VAR *)node;
		printf("Variable Node of entry %s\n", temp_var->entry->name);
		break;
	case FUNC_DECLS:
...
void ast_traversal(AST_Node *node)
{
	...
	else if (node->type == DO_UNTIL_NODE)
	{
		AST_Node_DO *temp_do_until = (struct AST_Node_DO *)node;
		ast_print_node(node);
		ast_traversal(temp_do_until->condition);
		ast_traversal(temp_do_until->do_branch);
	}
	/* assign case */
	...
	else if (node->type == FUNC_DECL)
	{
		...
		ast_print_node(node);
		if (temp_func_decl->entry->num_of_params != 0)
		{
			printf("Parameters:\n");
			for (i = 0; i < temp_func_decl->entry->num_of_params; i++)
			{
				printf("Parameter %s of type %d\n",
							 temp_func_decl->entry->params[i].name,
							 temp_func_decl->entry->params[i].type);
			}
		}
	...
	else if (node->type == DECL_PARAMS)
	{
		...
		{
			printf("Parameter %s of type %d\n", temp_decl_params->parameters[i].name, temp_decl_params->parameters[i].type);
		}
	}
	/* return case */
